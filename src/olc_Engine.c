...
olc_Sprite* olc_Sprite_LoadFromPGESprFile(const char *sImageFile)
{
    ...
    if(fp)
    {
        size_t wret = fread(&width, sizeof(int32_t), 1, fp);
        size_t hret = fread(&height, sizeof(int32_t), 1, fp);
        if(sizeof(width) != wret || sizeof(height) != hret)
        {
            fprintf(stderr, "Error loading PGESprite Header.\n");
            exit(EXIT_FAILURE);
        }
